...
# =============================
st_autorefresh(interval=70000, key="refresh_app")

# =============================
# INACTIVITY TIMEOUT (15 MIN)
# =============================
INACTIVITY_LIMIT = 30   # 900 sec = 15 minutes
...
# Check inactivity
now = time.time()
if st.session_state.get("logged_in") and (now - st.session_state.last_active) > INACTIVITY_LIMIT:
    st.session_state.clear()
    st.warning("⏳ You were logged out due to inactivity (15 minutes).")
    st.experimental_rerun()

# Update last_active on every rerun
st.session_state.last_active = time.time()
    ...
    if res.status_code == 200:
        ...
        if not df.empty:
            ...
            if avg_risk >= threshold:
                st.error(f"⚠ High risk! Your average risk {avg_risk} ≥ threshold {threshold}")
            elif avg_risk >= 0.4:
                st.warning(f"Moderate risk: {avg_risk}")
            else:
                st.success("Low risk — keep it up!")
        else:
            st.info("No records found for you.")
    else:
...
if role == "instructor":
    ...
    if r.status_code == 200:
        ...
        if not df.empty:
            ...
            c1, c2 = st.columns(2)
            with c1:
                fig = px.scatter(df, x="marks", y="attendance", color="risk_score")
                st.plotly_chart(fig, use_container_width=True)
            with c2:
                fig2 = px.histogram(df, x="risk_score", nbins=10)
    ...
    if a_res.status_code == 200:
        alerts = pd.DataFrame(a_res.json())
        if not alerts.empty and not df.empty:
            my_alerts = alerts[alerts['student_name'].isin(df['student_name'].tolist())]
            if not my_alerts.empty:
                st.markdown("### 🔔 Class Alerts")
                st.dataframe(my_alerts[['student_name','risk_score','created_at']])
    st.stop()
...
if role == "admin":
    ...
    new_thresh = st.slider("Risk threshold", 0.0, 1.0, cur_thresh, 0.05)
    ...
    new_r = st.selectbox("Role", ["student","instructor","admin"], 0)
